...
class NCO(nn.Module):
    ...
    def __init__(self, n_clusters, n_init=10, init="random", random_state=None):
        ...
        # self.weight_norm = WeightNorm(n_assets)
    ...
    def forward(self, covmat, rets=None):
        ...
        for i in range(n_samples):
            ...
            for c in range(self.n_clusters):
                ...
                intra_covmat = (
                    covmat[[i]].index_select(1, in_cluster).index_select(2, in_cluster)
                )  # (1, ?, ?)
                intra_rets = (
                    None if rets is None else rets[[i]].index_select(1, in_cluster)
                )  # (1, ?)
                w_intra_clusters[in_cluster, c] = self.analytical_markowitz_layer(
                    intra_covmat, intra_rets
                )[0]
                # NOTE: THIS HAS BEEN CHANGED BY ME.
                w_intra_clusters[in_cluster, c] = WeightNorm(len(in_cluster))(
            ...
            inter_rets = (
                None if rets is None else (w_intra_clusters.T @ rets[i]).view(1, -1)
            )  # (1, n_clusters)
            w_inter_clusters = self.analytical_markowitz_layer(
                inter_covmat.view(1, self.n_clusters, self.n_clusters), inter_rets
            )  # (1, n_clusters)
            w_final = (w_intra_clusters * w_inter_clusters).sum(dim=1)  # (n_assets,)
            # NOTE: THIS HAS BEEN CHANGED BY ME.
            w_final = WeightNorm(len(w_final))(w_final)[0]
...
class NumericalNCO(nn.Module):
    ...
    def __init__(
        ...
        self.portfolio_opt_layer_intra = SoftmaxAllocator(temperature=self.temperature1)
        self.portfolio_opt_layer_inter = SoftmaxAllocator(temperature=self.temperature2)

    def forward(self, gamma, alpha, covmat, rets=None):
        """Perform forward pass.
        ...
        for i in range(n_samples):
            ...
            for c in range(self.n_clusters):
                ...
                w_intra_clusters[in_cluster, c] = self.portfolio_opt_layer_inter(
                    intra_rets, intra_covmat, gamma, alpha
                )[0]
            ...
            w_inter_clusters = self.portfolio_opt_layer(
                inter_rets,
...
class NumericalMarkowitz(nn.Module):
    ...
    def __init__(self, n_assets, max_weight=1):
        ...
        prob = cp.Problem(
            cp.Maximize(ret - risk - reg), [cp.sum(w) == 1, w >= 0, w <= max_weight]
        )
...
class Resample(nn.Module):
    ...
    def __init__(
        self, allocator, n_draws=None, n_portfolios=5, sqrt=False, random_state=None
    ):
        super().__init__()
...
class SoftmaxAllocator(torch.nn.Module):
    ...
    def __init__(
        self, temperature=1, formulation="analytical", n_assets=None, max_weight=1
    ):
        super().__init__()
    ...
    def forward(self, x, temperature=None):
        ...
        return (
            self.layer(inp) if self.formulation == "analytical" else self.layer(inp)[0]
        )
